...
| Cache entry expiry  | per-entry `expires_in` / `expires_at`   | **global expiry only** — `max_age` + `max_size` FIFO trim; no per-entry TTL (planned, see `ROADMAP.md`) |
...
See `ROADMAP.md` in the repo root for the roadmap — the six things planned next. Everything

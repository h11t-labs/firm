"""namespace the cache table to firm_cache_entries

Revision ID: 0002
Revises: 0001
Create Date: 2026-07-21

Renames the released ``firm_entries`` table and its three secondary indexes to the workspace
``firm_<module>_<entity>`` convention — ``firm_entries`` → ``firm_cache_entries``,
``index_firm_entries_on_key_hash`` → ``index_firm_cache_entries_on_key_hash``, and so on.
...
under its new ``firm_cache_entries`` name and this revision skips the rename. Each step is guarded
with the inspector in online mode: a fresh database already has the new name and skips everything,
while a real pre-existing 0.x database still carries ``firm_entries`` and is renamed in place with
its rows intact.
...
_NEW_TABLE_PREFIX = "firm_cache_"
_OLD_INDEX_PREFIX = "index_firm_"
_NEW_INDEX_PREFIX = "index_firm_cache_"

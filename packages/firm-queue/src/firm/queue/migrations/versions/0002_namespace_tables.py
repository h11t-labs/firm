"""namespace tables to firm_queue_<entity>

Revision ID: 0002
Revises: 0001
Create Date: 2026-07-21

Renames every firm-queue table (and its ``firm_processes`` core companion) plus all their
secondary indexes from the released ``firm_<entity>`` names to the workspace
``firm_<module>_<entity>`` convention — ``firm_jobs`` → ``firm_queue_jobs``,
``index_firm_jobs_on_class_name`` → ``index_firm_queue_jobs_on_class_name``, and so on.

**Idempotent by design.** The 0001 baseline provisions the schema with ``metadata.create_all``,
which reflects the *current* metadata — so on a freshly built chain 0001 already creates every
table under its new ``firm_queue_*`` name and this revision must skip the rename. Each step is
guarded with the inspector in online mode: a fresh database already has the new names and skips
everything, while a real pre-existing 0.x database still carries the old ``firm_*`` names and is
renamed in place with its rows intact.
...
_NEW_TABLE_PREFIX = "firm_queue_"
_OLD_INDEX_PREFIX = "index_firm_"
_NEW_INDEX_PREFIX = "index_firm_queue_"

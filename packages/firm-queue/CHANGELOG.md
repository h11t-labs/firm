...
- `firm.queue.queries`: the read-query layer the dashboard used to keep to itself, now a supported
  ...
  `jobs_by_state`, `job_detail`, `processes`, `recurring`, and `STATES`. Each takes a SQLAlchemy
  `Connection` and returns plain dicts. A negative `limit`/`offset` or an unknown `state` now
  raises `ValueError` instead of a bare `KeyError` or a backend-specific result.

...
- `firm[cache]`, `firm[encryption]`, `firm[msgpack]` and `firm[all]` now require
  **`firm-cache~=1.1`**, so they deliver the `MsgpackCoder` that `firm[msgpack]` advertises.
  Raising the floor is what makes `pip install -U firm` actually move an installed `firm-cache`
  along: pip upgrades a dependency only when the new release requires it.
